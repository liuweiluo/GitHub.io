...
### 输出文件名Hash
一般我们部署资源文件时都会启用服务器的静态资源缓存，对于用户浏览器而言就可以缓存静态资源，后期就不需请求服务器去请求这些资源文件了，这样响应速度就有大幅度提升。
但缓存设置时间过短那效果就不明显，若设置时间过长应用重新部署不能被即时更新至客户端。为了解决这问题，我们在生产环境下文件名可使用Hash，对于客户端而言全新文件名就是全新请求。

webpack和绝大数插件中的filename属性都支持Hash，Hash也分为3种（hash，chunkhash，contenthash）

- hash：项目级别，一旦项目中有任何一个地方发生改动，打包后的文件hash值都会发生改变且都是一样的
- chunkhash：chunk（模块）级别，即如果文件发生改动，同一个chunk里的文件hash值会发生改变且都是一样，（同一个模块，就算将js和css分离，其哈希值也是相同的），引用chunk的文件hash值也会发生改变（被动改变，因为引用的文件名(路径)改变了，引用chunk的文件也会改变）
- contenthash：文件级别，不同文件就有不同hash值，如果文件发生改动，只有当前文件的hash会发生改变（由于文件名(路径)改变了，引用其文件hash值也会发生改变），这种也是解决缓存最好的方式，因为他精确到文件级别的hash
