...
1.安装 single-spa 脚手架工具： npm install create-single-spa@4.1.2  (这里可能有坑，容器应用里的webpack-config-single-spa 我是重新安装 4.0.0版本 )
...
当前项目中用于配合 single-spa Inspector （需用安装）调试工具使用.
...
2.创建不基于框架的微应用
  ...
  export async function mount() {
    ...
    lagouContainer.id = "xxxContainer"
    lagouContainer.innerHTML = "Hello xxx"
    document.body.appendChild(xxxContainer)
  }
  export async function unmount() {
    console.log("应用正在卸载")
    document.body.removeChild(xxxContainer)
  }
  ```
    ...
    <script type="systemjs-importmap">
      {
        "imports": {
          ...
          "@study/box": "//localhost:9001/study-box.js",
        }
      }
    </script>
  ```
